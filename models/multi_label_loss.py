...
class MyLoss(object):
    ...
    def _categorical_ghm_loss(bins=30, momentum=0.75):
        ...
        def ghm_class_loss(y_truth, y_pred, valid_mask):
            ...
            grads_bin = tf.logical_and(tf.greater_equal(
                gradient, edges[:-1, :, :]), tf.less(gradient, edges[1:, :, :]))
            valid_bin = tf.boolean_mask(
                grads_bin, valid_mask, name='valid_gradient', axis=1)
            valid_bin = tf.reduce_sum(
                tf.cast(valid_bin, dtype=tf.float32), axis=(1, 2))
            # 2. 更新指数移动平均后的梯度分布：S_ind(g)
            nonlocal acc_sum
            acc_sum = tf.add(momentum * acc_sum, (1 - momentum)
                             * valid_bin, name='update_bin_number')
    ...
    def categorical_crossentropy(self, y_truth, y_pred):
        ...
        :return: loss
        """
        num_classes = keras.backend.cast(
            keras.backend.int_shape(y_pred)[-1], dtype=tf.int32)  # 类别数
        ...
        y_truth = keras.backend.one_hot(
            indices=y_truth, num_classes=num_classes)

        # 标签平滑
        if self.is_label_smoothing:
            num_classes = keras.backend.cast(num_classes, dtype=y_pred.dtype)
            y_truth = (1.0 - self.smoothing_epsilon) * y_truth + \
                self.smoothing_epsilon / num_classes
