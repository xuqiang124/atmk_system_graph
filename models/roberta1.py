...
from keras.layers import Input, Dense, Embedding, LayerNormalization, Dropout, Concatenate, Reshape
import numpy as np
...
class Classifier(object):
    ...
    def build(cls, config, use_att=False, label_emb_matrix=None, metrics=None):
        ...
        def _multi_head_attention(query, key, value, num_heads=8, name=""):
            """
            多头注意力机制
            
            Args:
                query: [batch_size, query_length, hidden_size] 查询张量
                key: [batch_size, key_length, hidden_size] 键张量
                value: [batch_size, key_length, hidden_size] 值张量
                num_heads: 注意力头数量
                name: 作用域名称
                
            Returns:
                output: [batch_size, query_length, hidden_size] 注意力输出
                attention_weights: [batch_size, num_heads, query_length, key_length] 注意力权重
            """
            ...
            # 线性变换
            q = Dense(hidden_size, name=f"{name}_query")(query)  # (batch_size, query_length, hidden_size)
            k = Dense(hidden_size, name=f"{name}_key")(key)      # (batch_size, key_length, hidden_size)
            v = Dense(hidden_size, name=f"{name}_value")(value)  # (batch_size, key_length, hidden_size)
            
            # 分割成多头
            q = tf.reshape(q, [batch_size, -1, num_heads, depth])  # (batch_size, query_length, num_heads, depth)
            k = tf.reshape(k, [batch_size, -1, num_heads, depth])  # (batch_size, key_length, num_heads, depth)
            v = tf.reshape(v, [batch_size, -1, num_heads, depth])  # (batch_size, key_length, num_heads, depth)
            
            # 转置以便进行批量矩阵乘法
            q = tf.transpose(q, perm=[0, 2, 1, 3])  # (batch_size, num_heads, query_length, depth)
            k = tf.transpose(k, perm=[0, 2, 1, 3])  # (batch_size, num_heads, key_length, depth)
            v = tf.transpose(v, perm=[0, 2, 1, 3])  # (batch_size, num_heads, key_length, depth)
            
            # 计算注意力分数
            matmul_qk = tf.matmul(q, k, transpose_b=True)  # (batch_size, num_heads, query_length, key_length)
            ...
            # Softmax归一化
            attention_weights = tf.nn.softmax(scaled_attention_logits, axis=-1)  # (batch_size, num_heads, query_length, key_length)
            
            # 应用注意力权重
            output = tf.matmul(attention_weights, v)  # (batch_size, num_heads, query_length, depth)
            
            # 转置回原始形状
            output = tf.transpose(output, perm=[0, 2, 1, 3])  # (batch_size, query_length, num_heads, depth)
            
            # 合并多头
            output = tf.reshape(output, [batch_size, -1, hidden_size])  # (batch_size, query_length, hidden_size)
            
            # 最终线性变换
            output = Dense(hidden_size, name=f"{name}_output")(output)
        ...
        def _attention(input_x, label_emb_matrix, name=""):
            ...
            """
            # 使用多头注意力机制
            weighted_text_representation, attention_weights = _multi_head_attention(
                label_emb_matrix,  # 查询：标签嵌入
                input_x,           # 键：文本表示
                input_x,           # 值：文本表示
                num_heads=8,
            ...
            # 提取注意力权重，取平均值作为最终权重
            # attention_weights形状: [batch_size, num_heads, num_classes, max_length]
            attention_weight_probs = tf.reduce_mean(attention_weights, axis=1)  # [batch_size, num_classes, max_length]
        ...
        return model, roberta_output, label_emb[:, -num_classes_list[-1]:, :]s